...
a = {}
a[0] = (np.shape(np.array(spikes['L23exc'].i)))
a[1] = (np.shape(np.array(spikes['L23nfs'].i)))
a[2] = (np.shape(np.array(spikes['L23fs'].i)))
a[3] = (np.shape(np.array(spikes['L4exc'].i)))
a[4] = (np.shape(np.array(spikes['L4nfs'].i)))
a[5] = (np.shape(np.array(spikes['L4fs'].i)))

with open("test_fixed_nospike.txt", "a") as myfile:
    json.dump(a,myfile)

pops_net2.restore('nospike')
for k in np.random.randint(1500,size=1):
    pops['L4exc'].v[k] = 0*b2.mV
pops_net2.run(500*b2.ms, report = 'text')
b = {}
b[0] = (np.shape(np.array(spikes['L23exc'].i)))
b[1] = (np.shape(np.array(spikes['L23nfs'].i)))
b[2] = (np.shape(np.array(spikes['L23fs'].i)))
b[3] = (np.shape(np.array(spikes['L4exc'].i)))
b[4] = (np.shape(np.array(spikes['L4nfs'].i)))
b[5] = (np.shape(np.array(spikes['L4fs'].i)))


with open("test_fixed_spike.txt", "a") as myfile:
    json.dump(b,myfile)